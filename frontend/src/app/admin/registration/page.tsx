...
function AdminRegistration() {
  const [formData, setFormData] = useState({
    email: '',
    username: '',
    name: '', // Добавляем новое поле "name"
    password: '',
  ...
  const [formErrors, setFormErrors] = useState({
    email: '',
    username: '',
    name: '', // Ошибки для поля "name"
    password: '',
    server: '', // Поле для ошибок, приходящих с сервера
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      try {
        await createUser(formData); // Send form data to the server
        ...
        setFormErrors({ ...formErrors, server: serverError }); // Update server error
